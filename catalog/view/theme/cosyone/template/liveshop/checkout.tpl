  ...
  <div class="row"><?php echo $column_left; ?>
    ...
    <div id="content" class="<?php echo $class; ?>"><?php echo $content_top; ?>
      ...
      <div class="panel-group" id="accordion">
        ...
        <div class="panel panel-default">
          <div class="panel-heading">
            <h4 class="panel-title"><?php echo $text_checkout_referrer_options; ?></h4>
          </div>
          <div class="panel-collapse collapse" id="collapse-referrer-options">
            <div class="panel-body"></div>
          </div>
        </div>
        <?php } else if ($logged && $type == 'external') { ?>
		<div class="panel panel-default">
          <div class="panel-heading">
            <h4 class="panel-title"><?php echo $text_checkout_referrer_options; ?></h4>
          </div>
...
$(document).delegate('#button-login', 'click', function() {
    $.ajax({
        url: 'index.php?route=liveshop/login/save',
        type: 'post',
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
            	...
            	// updated by kareem
            	<?php if($logged && $type == 'external') { ?>
                $.ajax({
                    url: 'index.php?route=liveshop/referrer_options',
                    dataType: 'html',
                    success: function(html) {
                        $('#collapse-referrer-options .panel-body').html(html);
						
						$('#collapse-referrer-options').parent().find('.panel-heading .panel-title').html('<a href="#collapse-referrer-options" data-toggle="collapse" data-parent="#accordion" class="accordion-toggle"><?php echo $text_checkout_referrer_options; ?> <i class="fa fa-caret-down"></i>');
						
						$('a[href=\'#collapse-referrer-options\']').trigger('click');
                ...
                <?php } else if (($logged && $type == 'livefree')|| ($logged && $type == 'livetoursplus')) { ?>
                			<?php if ($shipping_required) { ?>
                $.ajax({
                    url: 'index.php?route=checkout/shipping_address',
                    dataType: 'html',
...
// updated by kareem
// Set Referrer
$(document).delegate('#button-referrer-options', 'click', function() {
    $.ajax({
        url: 'index.php?route=liveshop/referrer_options/save',
        type: 'post',
        data: $('#collapse-referrer-options input[type=\'radio\']:checked, #collapse-referrer-options input[type=\'text\']'),
        dataType: 'json',
        beforeSend: function() {
       		$('#button-referrer-options').button('loading');
	    },  
        complete: function() {
		$('#button-referrer-options').button('reset');
        },          
        success: function(json) {
            $('.alert, .text-danger').remove();
            ...
            if (json['redirect']) {
                ...
                if (json['error']['warning']) {
                    $('#collapse-referrer-options .panel-body').prepend('<div class="alert alert-warning">' + json['error']['warning'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                }
                /*                
				for (i in json['error']) {
					var element = $('#input-payment-' + i.replace('_', '-'));
           ...
           //     user_full_name = json.username;
             //   if(user_full_name)
//$('#collapse-referrer-options .panel-body').prepend('<div class="alert alert-success">' + user_full_name + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>'); 
...
// End set Referrer
$(document).delegate('#check_live_account', 'click', function() {
    $.ajax({
        url: 'index.php?route=liveshop/referrer_options/save',
        type: 'post',
        data: $('#collapse-referrer-options input[type=\'radio\']:checked, #collapse-referrer-options input[type=\'text\']'),
        dataType: 'json',
        beforeSend: function() {
       		$('#check_live_accounts').button('loading');
	    },  
        complete: function() {
			$('#check_live_account').button('reset');
        },          
        success: function(json) {
            $('.alert, .text-danger').remove();
